...
2. 采样分母写入 `sample_config_map`，后续根入口立即生效；已准入样本保持原权重采完。
...
**注意**：采样反馈只看堆内存水位。当 `event_queue` 直接丢事件泄压时，事件未进入用户态堆，内存可能一直不达阈值，出现 `skipped=0` 但 `dropped` 很大的现象——这是"队列泄压掩盖了生产-消费失衡"，属已知边界。如需更早介入，可将近期丢失率纳入采样反馈。
